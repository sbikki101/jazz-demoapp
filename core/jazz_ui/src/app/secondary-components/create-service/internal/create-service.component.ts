    ...
    if (this.typeOfService == 'api') {
      ...
      if (this.eventExpression.type !== "awsEventsNone") {
        ...
        if (this.eventExpression.type === "dynamodb") {
          event["source"] = "arn:aws:dynamodb:us-west-2:{primaryAccount}:table/" + this.eventExpression.dynamoTable;
          event["action"] = "PutItem";
        } else if (this.eventExpression.type === "kinesis") {
          event["source"] = "arn:aws:kinesis:us-west-2:{primaryAccount}:stream/" + this.eventExpression.streamARN;
